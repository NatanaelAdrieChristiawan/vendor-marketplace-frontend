...
const status = computed(() => {
  const id = orderId.value.toString().toUpperCase()
  if (id.includes('9901')) return 'BARU'
  if (id.includes('REVISI')) return 'REVISI'
  if (id.includes('DISPUTE')) return 'DISPUTE'
  if (id.includes('SELESAI')) return 'SELESAI'
  if (id.includes('BATAL')) return 'BATAL'
  return 'PROSES'
})
...
<template>
  <div class="p-12 space-y-10 bg-white min-h-screen pb-20 font-sans">
    <div v-if="status === 'BATAL'" class="bg-[#FEF2F2] border border-red-100 rounded-2xl p-10 flex items-start gap-8 animate-in fade-in slide-in-from-top-4 duration-500 mb-10">
      ...
      <div class="space-y-4">
        <div class="space-y-1">
          <h3 class="text-xl font-bold text-[#991B1B]">Kasus Ditutup</h3>
          <p class="text-sm font-bold text-[#991B1B]/60">REJECT-COMPLAINT - 30 April 2026, 14:22</p>
        </div>
    ...
    <div v-if="status === 'SELESAI'" class="bg-[#F0FDF4] border border-green-100 rounded-2xl p-10 flex items-start gap-8 animate-in fade-in slide-in-from-top-4 duration-500 mb-10">
      ...
      <div class="space-y-4">
        <div class="space-y-1">
          <h3 class="text-xl font-bold text-[#166534]">Kamu memenangkan kasus ini</h3>
          <p class="text-sm font-bold text-[#166534]/60">REJECT-COMPLAINT - 30 April 2026, 14:22</p>
        </div>
    ...
    <div v-if="status === 'DISPUTE'" class="bg-[#FFF7ED] border border-orange-100 rounded-2xl p-8 flex items-center gap-6 animate-in fade-in slide-in-from-top-4 duration-500 mb-10">
      ...
      <div>
        ...
        <p class="text-sm font-medium text-[#9A3412] opacity-80">Dana sebesar Rp750.000 telah dibekukan sementara</p>
      </div>
    </div>
    ...
    <div class="space-y-4">
      ...
      <div class="flex items-center justify-between">
        <div class="space-y-1">
          <div v-if="status === 'BATAL'" class="text-xs font-bold text-gray-400 flex items-center gap-2 mb-2">
            <span>Sengketa</span>
            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M9 5l7 7-7 7" /></svg>
            <span>#{{ orderId }}</span>
          </div>
          <p v-else class="text-sm font-bold text-[#1E3A8A] uppercase tracking-widest"># {{ orderId }}</p>
      ...
      <div class="col-span-8 space-y-12">
        ...
        <div class="bg-[#F8F9FE]/50 p-12 rounded-[40px] border border-gray-100 shadow-sm space-y-8">
          ...
          <div class="bg-white p-10 rounded-3xl border border-gray-100">
            <p class="text-sm font-medium text-gray-500 leading-loose">
        ...
        <div class="space-y-6">
          ...
          <div class="bg-white p-8 rounded-[32px] border border-gray-100 shadow-sm flex items-center justify-between group hover:border-[#4B6BFB]/20 transition-all">
            <div class="flex items-center gap-6">
              <div class="w-14 h-16 bg-red-50 rounded-xl flex items-center justify-center text-red-500 relative">
                ...
                <span class="absolute text-[8px] font-black mt-2">PDF</span>
              </div>
              <div class="space-y-1">
                <h4 class="font-bold text-gray-900">Guideline_Brand</h4>
                <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">1.2 MB</p>
              </div>
            </div>
            <button class="text-gray-400 hover:text-[#4B6BFB] transition-colors p-2">
        ...
        <div v-if="status === 'PROSES' || status === 'REVISI'" class="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
          <div class="space-y-4">
            <h3 class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">PENGUMPULAN HASIL PEKERJAAN</h3>
            <div class="border-2 border-dashed border-gray-100 rounded-[32px] p-16 flex flex-col items-center gap-4 hover:border-[#4B6BFB]/30 transition-all cursor-pointer bg-[#F8F9FE]/30 group">
          ...
          <div class="space-y-6 text-center">
            <button class="w-full py-5 bg-[#5BB0FF] text-white rounded-2xl text-lg font-black shadow-xl shadow-[#5BB0FF]/20 hover:bg-[#4DA3FF] transition-all">Kirim Hasil</button>
      ...
      <div class="col-span-4 space-y-8">
        <div class="bg-white p-10 rounded-[40px] border border-gray-100 shadow-sm space-y-10">
          <div class="space-y-6">
            ...
            <div class="flex items-center gap-4">
              <img src="https://i.pravatar.cc/150?u=budi" class="w-14 h-14 rounded-2xl object-cover" />
              <h4 class="text-lg font-bold text-[#1E3A8A]">Budi Santoso</h4>
            </div>
          </div>
          ...
          <div class="space-y-6 border-t border-gray-50 pt-8">
            ...
            <div class="flex justify-between items-center">
              <span class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">TOTAL PEMBARAYAN</span>
              <span class="text-xl font-black text-[#1E3A8A]">Rp150.000</span>
            </div>
          </div>
        ...
        <div v-if="status === 'BARU'" class="space-y-4">
          <button class="w-full py-5 bg-[#65A30D] text-white rounded-2xl text-base font-black shadow-xl shadow-green-500/20 hover:bg-[#4D7C0F] transition-all">TERIMA PESANAN</button>
          <button class="w-full py-5 bg-[#C2410C] text-white rounded-2xl text-base font-black shadow-xl shadow-red-500/20 hover:bg-[#9A3412] transition-all">TOLAK</button>
        </div>
        ...
        <div v-if="status === 'REVISI'" class="bg-white p-8 rounded-[40px] border border-gray-100 shadow-sm space-y-8 animate-in fade-in slide-in-from-right-4 duration-700">
          ...
          <div class="space-y-8">
            <div class="bg-white border border-gray-50 rounded-3xl p-8 shadow-sm space-y-4">
              <div class="flex justify-between items-center">
            ...
            <div class="space-y-6 pl-2 text-gray-400">
              <div class="flex gap-4 items-start relative">
                ...
                <div class="space-y-0.5 pb-2">
                  ...
                  <p class="text-[10px] font-bold">Dikirim pada 21 Okt 2026</p>
                </div>
              </div>
              <div class="flex gap-4 items-start">
                ...
                <div class="space-y-0.5">
                  ...
                  <p class="text-[10px] font-bold">Ditugaskan pada 18 Okt 2026</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <div class="grid grid-cols-2 gap-10">
        <div class="bg-white p-10 rounded-[40px] border border-gray-100 shadow-sm space-y-8">
          ...
          <div class="bg-gray-50/50 p-8 rounded-[32px] border border-gray-100 space-y-4">
            ...
            <div class="flex items-center justify-between">
              <h2 class="text-4xl font-black text-[#1E3A8A]">Rp 750.000</h2>
              <span class="px-4 py-1.5 bg-orange-50 text-orange-500 text-[10px] font-bold rounded-lg tracking-widest uppercase">IN_REVISION</span>
            </div>
          </div>
        </div>
        ...
        <div class="bg-white p-10 rounded-[40px] border border-gray-100 shadow-sm space-y-8">
          ...
          <div class="space-y-6">
            <div class="flex justify-between items-center border-b border-gray-50 pb-4">
              ...
              <span class="text-sm font-black text-[#1E3A8A]">28 April 2026, 09:14</span>
            </div>
            <div class="space-y-3">
              <span class="text-sm font-medium text-gray-500">Alasan client</span>
              <p class="text-base font-black text-gray-800 italic">"Hasil tidk sesuai brief"</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="space-y-12 bg-gray-50/30 p-12 rounded-[32px] border border-gray-50">
          <div class="flex flex-col gap-8 max-w-2xl mx-auto">
            <div class="self-start max-w-[80%] bg-[#E6F0FF] p-6 rounded-[32px] rounded-bl-none text-sm font-medium text-[#1E3A8A]">
              ...
              <div class="text-[10px] opacity-40 mt-2">04:45 PM</div>
            </div>
            <div class="self-end max-w-[80%] bg-[#4B6BFB] p-6 rounded-[32px] rounded-br-none text-sm font-medium text-white shadow-xl shadow-[#4B6BFB]/20">
              Oh, hello! All perfectly. I will check it and get back to you soon
              <div class="text-[10px] opacity-60 mt-2 text-right">04:45 PM</div>
            </div>
          </div>

          <div class="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm flex items-center justify-between group hover:border-[#4B6BFB]/20 transition-all max-w-4xl mx-auto">
            <div class="flex items-center gap-6">
              <div class="w-14 h-14 bg-[#1E3A8A] rounded-xl flex items-center justify-center text-white">
                <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
              </div>
              <div class="space-y-1">
                <h4 class="font-bold text-gray-900">Bukti Chat</h4>
                <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">File Format: PNG   File Size: 1.5MB</p>
              </div>
            </div>
            <button class="text-gray-400 hover:text-[#4B6BFB] transition-colors p-2 bg-gray-50 rounded-xl">
              <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a2 2 0 002 2h12a2 2 0 002-2v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg>
            </button>
          </div>
        </div>
        ...
        <div class="bg-white p-10 rounded-[40px] border border-gray-100 shadow-sm space-y-8">
          ...
          <div class="space-y-6">
            <div class="flex justify-between items-center border-b border-gray-50 pb-4">
              ...
              <span class="text-sm font-black text-[#1E3A8A]">30 April 2026, 09:14</span>
            </div>
      ...
      <div class="grid grid-cols-2 gap-10">
        <div class="bg-white p-10 rounded-[40px] border border-gray-100 shadow-sm space-y-8">
          ...
          <div class="bg-red-50/50 p-8 rounded-[32px] border border-red-100 space-y-4">
            <p class="text-sm font-medium text-gray-500">Total nilai order</p>
            <div class="flex items-center justify-between">
              <h2 class="text-4xl font-black text-[#991B1B]">Rp 750.000</h2>

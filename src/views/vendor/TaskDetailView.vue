...
type OrderStatus = 'BARU' | 'PROSES' | 'DIKIRIM' | 'SELESAI' | 'REVISI'

// Mock order data
const order = ref({
  id: route.params.id || 'ORD-20240512-9901',
  ...
  instructions: 'Mohon buatkan desain logo yang memadukan elemen minimalis dan futuristik. Fokus pada penggunaan warna biru navy dan aksen perak. Klien menginginkan simbol yang merepresentasikan "Kecepatan".',
  attachments: [
    { name: 'Guideline_Brand', size: '1.2 MB', type: 'pdf' },
  ],
  revisionHistory: [
    {
      ...
      time: '2 Jam lalu',
      message: '"atur dikit warna nya jgn terlalu kontras"',
      attachment: 'ref_lighting_3.jpg',
    },
    {
      type: 'delivery',
      label: 'PENGIRIMAN AWAL',
      time: 'Dikirim pada 21 Okt 2026',
    },
    {
      type: 'started',
      label: 'PESANAN DIMULAI',
      time: 'Ditugaskan pada 18 Okt 2026',
    },
  ],
})
...
function submitDelivery() {
  order.value.status = 'DIKIRIM'
  deliveryFiles.value = []
  deliveryMessage.value = ''
}

// Mock: cycle through states for demo
function mockRevision() {
  order.value.status = 'REVISI'
}
</script>
  ...
  <div class="py-6 px-4 max-w-6xl">
    ...
    >
      ...
      <div>
        ...
        <p class="text-xs text-red-600/80">Tindakan diperlukan segera</p>
      </div>
    </div>

    <!-- Header -->
    <div class="mb-8">
      <p class="text-sm font-bold text-brand-light mb-1"># #{{ order.id }}</p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div class="lg:col-span-2 space-y-8">
        ...
        <div>
          ...
          <div class="space-y-3">
            ...
            >
              ...
              <button class="text-gray-400 hover:text-brand-navy transition-colors p-2">
        ...
        <!-- Delivery Section (PROSES / REVISI only) -->
        <div v-if="order.status === 'PROSES' || order.status === 'REVISI'" class="space-y-6">
      ...
      <div class="space-y-6">
        ...
        <div class="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm">
          ...
          <div class="flex items-center gap-3 mb-6">
            <img :src="order.buyer.avatar" :alt="order.buyer.name" class="w-12 h-12 rounded-full object-cover" />
            <h3 class="text-base font-bold text-gray-900">{{ order.buyer.name }}</h3>
          </div>

          <div class="space-y-4 mb-6">
            <template v-if="order.status === 'BARU'">
              <div class="flex justify-between items-center">
                <span class="text-xs text-gray-400 font-medium uppercase tracking-wider">Status Saat Ini</span>
                <span class="text-sm font-bold text-blue-600">Baru</span>
              </div>
            </template>
            <div class="flex justify-between items-center">
              <span class="text-xs text-gray-400 font-medium uppercase tracking-wider">Tenggat Waktu:</span>
              <span class="text-sm font-bold text-orange-500">{{ order.deadline }}</span>
            </div>
            <div class="flex justify-between items-center">
              <span class="text-xs text-gray-400 font-medium uppercase tracking-wider">Total Pembayaran</span>
              <span class="text-lg font-bold text-gray-900">{{ order.totalPayment }}</span>
            </div>
          </div>

          <!-- Action Buttons (Baru only) -->
          <template v-if="order.status === 'BARU'">
            <div class="space-y-3 border-t border-gray-100 pt-5">
              <button class="w-full py-3 border border-gray-200 rounded-xl text-sm font-bold text-gray-700 hover:bg-gray-50 transition-colors">
        ...
        <!-- Revision History (REVISI only) -->
        <div v-if="order.status === 'REVISI'" class="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm">
          <h3 class="text-base font-bold text-gray-900 mb-5">Riwayat Revisi</h3>
            ...
            <div class="space-y-6">
              ...
              >
                ...
                <div>
                  ...
                  <div v-if="entry.attachment" class="flex items-center gap-2 text-xs text-brand-light font-medium">
        ...
        <div v-if="showDeclineModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div class="relative bg-white rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden">
            ...
            <div class="p-6 space-y-5">
              <p class="text-sm text-gray-600">Berikan alasan penolakan. Uang akan di-refund ke client.</p>
              <div>
